...
1) Install from GitHub Releases (primary):
   https://github.com/sliverTwo/yunxiao-cli/releases/tag/v0.16.4
   Download the archive for the user's OS/arch, extract, put `yunxiao` on PATH.
   yunxiao --version   # expect 0.16.4
...
**Recommended — [GitHub Releases](https://github.com/sliverTwo/yunxiao-cli/releases/tag/v0.16.4):**

Download the archive for your OS/arch, extract it, and add the `yunxiao` binary to `PATH`.

```bash
yunxiao --version          # yunxiao 0.16.4
```
...
# or (without ldflags, Version falls back to package default 0.16.4)
go build -o yunxiao .
# pin version explicitly:
# go build -ldflags "-X github.com/yunxiao-cli/yunxiao/internal/version.Version=0.16.4" -o yunxiao .
make install               # installs to ~/.local/bin/yunxiao
# or
go install github.com/yunxiao-cli/yunxiao@latest   # when published
```

Requires Go 1.24.4+. `make build` / `make ci` set `-ldflags -X …version.Version=$(VERSION)` (`VERSION` defaults to `git describe` or `0.16.4`).
...
The hint itself is printed in Chinese, for example: `发现新版本 yunxiao：0.16.4 → 0.16.5。运行：yunxiao update`.
