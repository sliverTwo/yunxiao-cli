...
**推荐 — [GitHub Releases](https://github.com/sliverTwo/yunxiao-cli/releases/tag/v0.16.4)：**
...
go build -o yunxiao .   # 无 ldflags 时回退包内默认 0.16.4
# 显式注入：
# go build -ldflags "-X github.com/yunxiao-cli/yunxiao/internal/version.Version=0.16.4" -o yunxiao .
```

需要 Go 1.24.4+。`make build` / `make ci` 通过 `-ldflags -X …version.Version=$(VERSION)` 注入版本（`VERSION` 默认 `git describe` 或 `0.16.4`）。
...
提示示例（打印到 **stderr**）：`发现新版本 yunxiao：0.16.4 → 0.16.5。运行：yunxiao update`
